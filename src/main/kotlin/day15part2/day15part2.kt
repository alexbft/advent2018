...
private const val GOBLIN_ATTACK = 3
...
fun trySolve(areaMapRaw: List<String>, attack: Int): SolveResult? {
...
//        println(areaMap.joinToString("\n") { row -> row.joinToString("") { it.toChar() } })
        ...
        for (mob in mobs) {
            ...
            if (nearTarget != null) {
                ...
                if (!nearTarget.isAlive && nearTarget.team == TEAM_ELF) {
                    return null
                }
            }
        }
...
fun solveFull(areaMapRaw: List<String>): SolveResult {
    for (attack in GOBLIN_ATTACK..HP_MAX) {
        trySolve(areaMapRaw, attack)?.let { return it }
    }
