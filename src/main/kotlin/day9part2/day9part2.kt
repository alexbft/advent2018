...
fun solve(players: Int, maxMarble: Int): Long {
    val circle = Circle()
    circle.add(0)
    var player = 0
    val playerScore = Array(players) { 0L }
    for (marble in 1..maxMarble) {
        if (marble % 23 == 0) {
            playerScore[player] += marble.toLong()
            circle.moveBack(7)
            playerScore[player] += circle.removeCurrent().toLong()
        } else {
            circle.moveToNext()
            circle.add(marble)
        }
